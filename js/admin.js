// Kiểm tra và thiết lập tài khoản admin khi đăng nhập
function checkLogin() {
...
// Khởi tạo tài khoản admin mới nếu chưa tồn tại
function initializeAdminAccount() {
    let accounts = JSON.parse(localStorage.getItem("accounts")) || [];
    let adminExists = accounts.some(account => account.userType === 1); // Kiểm tra admin đã tồn tại
    ...
    if (!adminExists) {
        ...
        accounts.push({
            username: 'MinhMan',
            password: '123123',
            fullname: 'Admin MinhMan',
            userType: 1, // 1 là admin
            status: 1,
            join: new Date(),
        });
        ...
        accounts = accounts.map(account =>
            account.userType === 1
                ? { ...account, username: 'MinhMan', password: '123123', fullname: 'Admin MinhMẫn' }
                : account
        );
        localStorage.setItem("accounts", JSON.stringify(accounts));
    }
...
// Hiển thị số lượng sản phẩm, tài khoản, và doanh thu
...
function getAmoumtUser() {
    let accounts = JSON.parse(localStorage.getItem("accounts")) || [];
    return accounts.filter(account => account.userType === 0).length;
}

function getMoney() {
    let orders = JSON.parse(localStorage.getItem("order")) || [];
    return orders.reduce((total, order) => total + (order.tongtien || 0), 0);
}
